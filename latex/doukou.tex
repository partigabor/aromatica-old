...
\begin{abstract}
    This paper aims to provide a new etymological explanation for the Chinese word \tc{豆蔻} \textit{dòukòu} `cardamom; nutmeg' and explores the potential trade-language origins of the term. We propose that this word is a loanword that may have entered the Chinese lexicon via Southern Min, originating from an Indian Ocean trade-language used on the ancient Maritime Silk Road during the Tang era (618--907). Our findings suggests that this word entered the Chinese lexicon alongside the economic products it denotes, and underwent phono-semantic matching that obscures its foreign origins. We present our reasoning in four main points, detailing our observations regarding 1) the earliest written records, 2) character composition, 3) available lexicographical data, and 4) an overview of seemingly related regional \textit{Wanderwörter} (wandering loanwords) as evidence supporting our hypothesis. After elucidating the lexicogenesis of this word, we discuss the linguistic and historical plausibility of our claims, and explore potential candidate etymons, such as Prakrit and Arabic. 
    
    % Additionally, we clarify the situation surrounding cardamoms from an Asian perspective, focusing on their identities, origins, and terminology.
...
Cardamom is a popular spice with a history spanning thousands of years of culinary and medicinal use; the reader might recall the vibrant aroma of the green seed-pods. Less well-known is the fact however, that the name \textit{cardamom} can refer to multiple different kinds of fragrant fruits, besides the familiar pods of the ``true cardamom'', \textit{Elettaria cardamomum}. Furthermore, delving deeper into the identities of cardamoms from an Asian perspective reveals a complex array of words and materials, prototypical cardamoms are botanically distinct species found in various regions (e.g., Eastern Himalayas, Indochina, Java, Yunnan), and their names and identities have become somewhat entangled along the paths of diffusion.

In the present study, we examine the situation in Chinese, where the corresponding word is \tc{豆蔻} \textit{dòukòu}, also pointing towards an unrelated spice, nutmeg...
...
The paper also aims to clarify the situation surrounding the multitude of cardamoms in Asia, focusing on the identities and terminologies in a Chinese context, as the word\textit{dòukòu}, too, is ambiguous and can refer to several different aromatic fruits and seeds.

Setting the scene for our etymological investigation, we first provide some answers on what cardamom is, and why is it a question in the first place.
