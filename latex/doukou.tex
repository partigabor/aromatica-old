...
\begin{figure}[!ht]
	\vspace{-2ex}
	\centering
	\subfloat{\includegraphics[width=0.3\linewidth]{imgs/green_cardamom.png}}
	\hfill
	\subfloat{\includegraphics[width=0.3\linewidth]{imgs/black_cardamom.png}}
	\hfill
	\subfloat{\includegraphics[width=0.3\linewidth]{imgs/white_cardamom.png}}
...
The first mention of \textit{dòukòu} appears in the \tc{藝文類聚} \textit{Yiwen Leiju}, an encyclopedic work (\textit{leishu}) compiled in 624. It references earlier writings -- including poetry -- thus aiding later literary scholars to reconstruct otherwise lost books. In volume\footnote{\tc{卷} \textit{juàn}, technically meaning `scroll', or `book'.} 32, we find lines quoting a poem titled ``Spring farewell poem'' (\tc{春別詩}):\footnote{The text is accessible via the Chinese Text Project \parencite{sturgeon_2021_chinese}, see volume 32, section 18 of the \textit{Yiwen Leiju}: \url{https://ctext.org/text.pl?node=543270\&if=en\#n543288}; a free translation is provided by the authors.}

\begin{quote}

\tc{別觀蒲萄帶實垂，} Don't you see the grapes hang in clusters ripe,
...
% The poem is a part of the \tc{閨情} \textit{guīqíng} genre,

Those with hearts and regrets are us humans abandoning each other, unlike nature's ``unaware'' creatures that bind together. 
% The poem is a part of the \tc{閨情} \textit{guīqíng} genre, which is a type of poetry that was popular during the Tang dynasty, and it is characterized by its themes of love, separation, and longing ??.
According to the poem, cardamom plants of some sort are growing in Jiangnan, which is a historical region in the lower Yangtze River valley, known for its mild climate and  picturesque landscapes -- a typical backdrop for Tang poetry. It is not easy to tell if a type of cardamom\footnote{\textit{Wurfbania villosa} and \textit{Alpinia hainanensis} are native to South China.} was really cultivated here, or the invocation of aromatic plants is purely an artistic tool; \textit{dòukòu} this time could also refer to an exotic, imported spice. Nonetheless, the poem is a valuable piece of evidence for the early existence of the word \textit{dòukòu} in Chinese literature, and people's awareness of a fragrant plant called \textit{dòukòu} in the 7\textsuperscript{th} century.
